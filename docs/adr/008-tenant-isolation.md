# ADR-008 Decision to Physically & Logically Isolate Tenant Environments
...
The pill organizer may be used for adherence tracking as part of a clinical trials or institutional use (different tenants).
Tenants may have different access, data retention, and regulatory requirements compared to commercial use.
...
Each "tenant" has its own dedicated backend infrastructure to the highest extent practicable.
...
* Easier to attribute operational costs (i.e., AWS costs) to a particular tenant for billing purposes.
