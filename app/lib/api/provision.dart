...
class WifiEntry {
  ...
  factory WifiEntry.fromMap(Map<String, String> map) {
    return WifiEntry(
      ...
      security: map.containsKey("security") ? int.parse(map["security"]!) : null,
    );
  }
}
...
class ProvisionProvider extends ChangeNotifier {
  ...
  Future<ProvisionState> _scanWifi() async {
    var res = await _flutterEspBleProvPlugin
        .scanWifiNetworks(_state.deviceName!, _popKey);

    var list = res.map((e) => WifiEntry.fromMap(e))
        .toList(growable: false);

    _state = _state.copyWith(wifiNetworks: list, stage: ProvisionStage.select_wifi);
    notifyListeners();
    return _state;
  }
  ...
  Future<ProvisionState> scanWifi() {
    ...
    _timer = Timer.periodic(const Duration(milliseconds: 250), (timer) {
      ...
      _state = _state.copyWith(progress: (_state.progress ?? 0.0) + halfRemaining);
      notifyListeners();
    });
  ...
  Future<ProvisionState> _startProvisioning() async {
    for(int i = 0; i < 5; i++) {
      List<String> devices = await _flutterEspBleProvPlugin
          .scanBleDevices(_prefix);
  ...
  Future<ProvisionState> _setWifiPassword(BuildContext context, String ssid, String pw) async {
  ...
  Future<void> _exchangeProvisionKey(BuildContext context) async {
    ...
    var prov = await client.provisionStart(
        ProvisionStart(
            serialNo: sn,
            deviceClass: AppApi.deviceClass()
        )
    );
  ...
  Future<bool> _checkProvision() async {
    try {
      var p = await client.provisionStatus(
          _state.provisionID!,
  ...
  Future<ProvisionState> checkProvision() async {
    final int iterations = completionTimeout.inSeconds ~/
        completionCheckPeriod.inSeconds;
  ...
  Future<ProvisionState> setWifiPassword(BuildContext context, String ssid, String pw) {
    _state = _state.copyWith(
      ...
      future: _setWifiPassword(context, ssid, pw)
          ...
          .onError((err, st) {
            debugPrintStack(stackTrace: st, label: 'Set wifi password error: $err');
            _state = _state.copyWith(error: err, stage: ProvisionStage.select_wifi);
            notifyListeners();
            return Future.error(err!);
          })
    );
