...
class Boss {
   ...
   * callback. If already connected to the same device, reuse the existing connection.
   */
  fun connect(conn: BleConnector, deviceName: String, proofOfPossession: String, onConnectCallback: (ESPDevice) -> Unit) {
    if (activeDevice != null && connectedDeviceName == deviceName) {
      d("Reuse existing connection to $deviceName. Resuming session...")
      activeDevice?.proofOfPossession = proofOfPossession
      onConnectCallback(activeDevice!!)
      return
    }
    ...
    val listener = object {
      ...
      fun onEvent(event: DeviceConnectionEvent) {
        d("Bus event received: ${event.eventType}")
        when (event.eventType) {
          ESPConstants.EVENT_DEVICE_CONNECTED -> {
            ...
            d("Device connected, starting session initialization...")
            esp.initSession(object : ResponseListener {
                override fun onSuccess(returnData: ByteArray?) {
                    d("Session initialization successful")
                    onConnectCallback(esp)
                }
                
                override fun onFailure(e: Exception?) {
                    e("Session initialization failed: $e")
                    activeDevice = null
                    connectedDeviceName = null
                }
            })
          }
          ESPConstants.EVENT_DEVICE_DISCONNECTED, ESPConstants.EVENT_DEVICE_CONNECTION_FAILED -> {
            EventBus.getDefault().unregister(this)
            if (activeDevice == esp) {
              activeDevice = null
              connectedDeviceName = null
...
class WifiScanManager(boss: Boss) : ActionManager(boss) {
  override fun call(ctx: CallContext) {
    ...
    boss.connect(conn, name, proofOfPossession) { esp ->
...
class WifiProvisionManager(boss: Boss) : ActionManager(boss) {
  override fun call(ctx: CallContext) {
    ...
    boss.connect(conn, deviceName, proofOfPossession) { esp ->
...
class CustomDataManager(boss: Boss) : ActionManager(boss) {
  override fun call(ctx: CallContext) {
    ...
    val data = ctx.call.argument<ByteArray>("data") ?: ByteArray(0)
    val conn = boss.connector(deviceName) ?: return

    boss.d("sendCustomData: start path=$path")
    boss.connect(conn, deviceName, proofOfPossession) { esp ->
      // Standard ESP Provisioning often needs a small breather after session start
      Handler(Looper.getMainLooper()).postDelayed({
        val finalData = if (data.isEmpty()) byteArrayOf(0x00) else data
        boss.d("sendCustomData call: path=$path, dataSize=${finalData.size}")
        ...
        val timeoutRunnable = Runnable {
          if (!completed) {
            completed = true
            boss.e("sendCustomData TIMEOUT: $path")
            ctx.result.error("TIMEOUT", "ProtoComm call timed out in Kotlin", path)
          }
        }
        timeoutHandler.postDelayed(timeoutRunnable, 5000)

        esp.sendDataToCustomEndPoint(path, finalData, object : ResponseListener {
          override fun onSuccess(responseData: ByteArray?) {
            if (completed) return
            completed = true
            timeoutHandler.removeCallbacks(timeoutRunnable)
            boss.d("sendCustomData SUCCESS: path=$path responseSize=${responseData?.size ?: 0}")
            Handler(Looper.getMainLooper()).post {
              ctx.result.success(responseData)
            }
          }
          ...
          override fun onFailure(e: Exception?) {
            ...
            completed = true
            timeoutHandler.removeCallbacks(timeoutRunnable)
            boss.e("sendCustomData FAILURE: path=$path error=$e")
            Handler(Looper.getMainLooper()).post {
              ctx.result.error("E4", "Custom data send failed", "Exception details $e")
            }
          }
        })
      }, 200)
