...
public class SwiftFlutterEspBleProvPlugin: NSObject, FlutterPlugin {
    ...
    private let CUSTOM_ENDPOINT = "customEndpoint"
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        } else if(call.method == CUSTOM_ENDPOINT) {
            let deviceName = arguments["deviceName"] as! String
            let proofOfPossession = arguments["proofOfPossession"] as! String
            let endpoint = arguments["endpoint"] as! String
            let data = arguments["data"] as! String
            provisionService.customEndpoint(
                            deviceName: deviceName,
                            proofOfPossession: proofOfPossession,
                            endpoint: endpoint,
                            data: data
...
protocol ProvisionService {
    ...
    func customEndpoint(deviceName: String, proofOfPossession: String, endpoint: String, data: String) -> Void
}
    ...
    func scanWifiNetworks(deviceName: String, proofOfPossession: String) {
        self.connect(deviceName: deviceName, proofOfPossession: proofOfPossession) {
            ...
            device?.scanWifiList { wifiList, error in
                ...
                device?.disconnect()
                self.result(wifiList?.map({(networks: ESPWifiNetwork) -> Dictionary<String, String>
                    in return [
                        "name": networks.ssid,
                        "rssi": String(networks.rssi),
                        "security": String(networks.auth.rawValue)
                    ]}))
        ...
        self.connect(deviceName: deviceName, proofOfPossession: proofOfPossession){
            ...
            device?.provision(ssid: ssid, passPhrase: passphrase) { status in
                ...
                case .failure:
                    ...
                    self.result(false)
                case .configApplied:
                    NSLog("Configuration applied")
                }
            }
        }
    }

    func customEndpoint(deviceName: String, proofOfPossession: String, endpoint: String, data: String) {
        self.connect(deviceName: deviceName, proofOfPossession: proofOfPossession){
                        device in
                        device?.sendData(path: endpoint, data: Data(base64Encoded: data)!) { data, error in
                            device?.disconnect()
                            NSLog("Send data to \(endpoint)")
                            self.result(data)
                        }
        }
    }
