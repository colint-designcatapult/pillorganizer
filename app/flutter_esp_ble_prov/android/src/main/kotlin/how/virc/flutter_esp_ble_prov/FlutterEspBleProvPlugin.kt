...
class Boss {
  ...
  private val customEndpointMethod = "customEndpoint"
   ...
   */
  val networks = mutableSetOf<Map<String, String>>()
  ...
  private val customEndpoints: CustomEndpointManager = CustomEndpointManager(this)
  ...
  fun call(call: MethodCall, result: Result) {
    permissionManager.ensure(fun(_: Boolean) {
      ...
      when (call.method) {
        ...
        customEndpointMethod -> customEndpoints.call(ctx)
  ...
  override fun call(ctx: CallContext) {
    ...
    boss.espManager.searchBleEspDevices(prefix, object : BleScanListener {
      override fun scanStartFailed() {
        ctx.result.error("scanStartFailed", "scanStartFailed", "scanStartFailed");
      }
      ...
      override fun onFailure(e: java.lang.Exception?) {
        ctx.result.error(e.toString(), e.toString(), e.toString());
      }
    ...
    boss.connect(conn, proofOfPossession) { esp ->
      ...
      esp.scanNetworks(object : WiFiScanListener {
        override fun onWifiListReceived(wifiList: ArrayList<WiFiAccessPoint>?) {
          wifiList ?: return
          wifiList.forEach {
            boss.networks.add(mapOf("name" to it.wifiName,
                    "security" to it.security.toString(),
                    "rssi" to it.rssi.toString()))
          }
          boss.d("scanNetworks: complete ${boss.networks}")
          Handler(Looper.getMainLooper()).post {
            ctx.result.success(ArrayList<Map<String, String>>(boss.networks))
          }
...
class CustomEndpointManager(boss: Boss) : ActionManager(boss) {
  override fun call(ctx: CallContext) {
    boss.e("customEndpoint ${ctx.call.arguments}")
    val endpoint = ctx.arg("endpoint") ?: return
    ...
    boss.connect(conn, proofOfPossession) { esp ->
      boss.d("customEndpoint ${endpoint}: start")
      esp.sendDataToCustomEndPoint(endpoint, Base64.decode(data, 0), object : ResponseListener {
        override fun onSuccess(returnData: ByteArray?) {
          boss.d("customEndpoint ${endpoint}: success")
          ctx.result.success(returnData)
          esp.disconnectDevice();
        }

        override fun onFailure(e: Exception?) {
          boss.d("customEndpoint ${endpoint}: fail ${e}")
