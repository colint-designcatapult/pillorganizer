...
public class AppDeviceAPIController {
    ...
    @Secured(SecurityRule.IS_AUTHENTICATED)
    // todo: authz check here (can an arbitrary user access an arbitrary provision record?)
    public HttpResponse<?> checkProvisionStatus(@QueryValue long id, @Body VerifyProvision vp) {
        ...
        // todo: change this so we don't just return 200 OK and use exceptions to handle not provisioned yet
    ...
    @Operation(summary = "Updates device basic settings",
            description = "Updates non-schedule settings for a device, including timezone, name, and notification token.")
    @Put("/{id}")
    @Secured(SecurityRule.IS_AUTHENTICATED)
    @DeviceABAC
