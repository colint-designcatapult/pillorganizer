...
public class DeviceAPIv12Controller {
    ...
    @Operation(summary = "Syncs a device's state",
        description = "Performs a two-way device sync, accepting a device's state, processing events, and returning " +
    ...
    public HttpResponse<?> sync(@Body byte[] body) throws InvalidProtocolBufferException {
        ...
        return HttpResponse.ok(
                deviceStateService
                        .wrapperOf(deviceAuthService.getDevice())
                        .sync(req)
                        .toByteArray()
        );
    }

    @Operation(summary = "Completes the provisioning process",
            description = "Indicates that the device has been successfully provisioned and connected to WiFi. The " +
